...
//
// #define JAY_SIGSEGV_HANDLER
// - Installs a signal handler for SIGSEGV and prints a jay stack trace when we segfault.
//
// #define JAY_SIGSEGV_STACKCHECK
// - When combined with JAY_SIGSEGV_HANDLER, makes stack checking more efficient, as we no 
//   longer have to explicitly perform stack checks; instead, we simply segfault if we 
//   overflow the stack and then have the signal handler safely panic
//   (It can show a stack overflow). Note that this is not always supported -- the Linux
//   kernel has to be able to correctly tell us the faulting address for it to work.
...
// Field "names" used in our hashmap. This is distinct from the string
// names created by JAY_ENABLE_NAMES.
typedef uint32_t jay_name;

// If we have name support, then anything that wants a name gets a
// const char *name field.
#ifdef JAY_ENABLE_NAMES
	#define JAY_HAS_NAME const char *name;
	static const char* jay_name_to_string(jay_name name);
	// Extra parameter on functions that take names
	#define JAY_NAME_PARAMETER , const char *name
	// Argument to those parameters generated by jaylox compiler
	#define JAY_NAME_ARGUMENT(name_str) , name_str
	#define JAY_NAME_NULL , NULL
	// Assign those names inside those functions
	#define JAY_ASSIGN_NAME(obj_ptr) (obj_ptr)->name = name;
#else
	#define JAY_HAS_NAME

	// These all get compiled to nothing when names are disabled.
	#define JAY_NAME_PARAMETER
	#define JAY_NAME_ARGUMENT(name_str)
	#define JAY_NAME_NULL
	#define JAY_ASSIGN_NAME(obj_ptr)
#endif
...
switch(JAY_TAG(value)) {
		...
		case JAY_FALSE:
			...
			break;
		case JAY_INSTANCE:
			printf("%s instance\n", JAY_AS_INSTANCE(value)->class->name);
			break;
		case JAY_CLASS:
			printf("%s\n", JAY_AS_CLASS(value)->name);
			break;
		case JAY_BOUND_METHOD:
			printf("<fn %s>\n", JAY_AS_BOUND_METHOD(value)->name);
			break;
		case JAY_FUNCTION: {
			const char *name = JAY_AS_FUNCTION(value)->name;
			if(!name) {
				// To distinguish native fns, give them null name.
				// We could just actually give them a name, but this gives
				// us better Lox conformance.
				puts("<native fn>");
			}
			else {
				printf("<fn %s>\n", name);
...
void
jay_sigsegv_handler(int sig, siginfo_t *info, void *ucontext) {
	static bool has_entered_sig = false;
	// If we've already entered the signal handler, that must mean some sort
	// of recursive segfault e.g. when we try to oops. Just immediately exit.
	if(has_entered_sig) {
		exit(1);
	}
	has_entered_sig = true;

	uintptr_t faultaddr = (uintptr_t)info->si_addr;

	uintptr_t stack_s = (uintptr_t)(jay_stack);
	uintptr_t frames_s = (uintptr_t)(jay_frames);
	uintptr_t harbor_s = (uintptr_t)(jay_harbor_stack);
	uintptr_t stack_e = (uintptr_t)(&jay_stack[JAY_STACK_SIZE]);
	uintptr_t frames_e = (uintptr_t)(&jay_frames[JAY_FRAMES_SIZE]);
	uintptr_t harbor_e = (uintptr_t)(&jay_harbor_stack[JAY_HARBOR_SIZE]);

	// Probably fault in jay_stack.
	if(faultaddr >= stack_s && faultaddr - 4096 <= stack_e) {
		oops("Stack overflow (jay_stack).");
	}
	// Probably fault in jay_frames.
	else if(faultaddr >= frames_s && faultaddr - 4096 <= frames_e) {
		// If we faulted in jay_frames, that means that the frames_ptr is likely
		// pointing to an invalid frame. So instead, we should reset frames_ptr
		// to the real end of the array and then print the backtrace from there.
		oops("Stack overflow (jay_frames).");
	}
	// Probably fault in jay_harbor.
	else if(faultaddr >= harbor_s && faultaddr - 4096 <= harbor_e) {
		oops("Stack overflow (jay_harbor).");
	}
	// Not a stack overflow. General segfault.
	else {
		oops("Segmentation fault at address %p.", faultaddr);
	}
}

#define jay_sigsegv_install() do { \
	struct sigaction segv_act = {0}; \
	segv_act.sa_sigaction = jay_sigsegv_handler; \
	segv_act.sa_flags = SA_SIGINFO | SA_RESETHAND; \
	sigaction(SIGSEGV, &segv_act, NULL); \
} while(0)

#else

#define jay_sigsegv_install()
...
void
jay_harbor(void *ptr) {
#if !defined(JAY_SIGSEGV_HANDLER) || !defined(JAY_SIGSEGV_STACKCHECK) // Only need to check if we aren't handling this with a signal handler.
...
#if defined(JAY_SIGSEGV_HANDLER) && defined(JAY_SIGSEGV_STACKCHECK)

// Note that JAY_STACK_CHECK() is generated by the compiler when it does "virtual pushes".
// We can delete all of these when we're using the signal handler because it will detect
// the stack overflows for us.
#define JAY_STACK_CHECK()
...
#ifdef JAY_ENABLE_NAMES
	oops("Undefined property '%s'.", jay_name_to_string(name));
#else
	oops("Tried to get undefined property.");
#endif
}
...
jay_op_invoke(size_t name, size_t arity, jay_instance *instance) {
	...
	if(!method) {
		...
		oops("Undefined property '%s'.", jay_name_to_string(name));
#else
		oops("Tried to invoke undefined property.");
#endif
	}
